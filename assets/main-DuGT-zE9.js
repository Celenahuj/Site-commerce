...
</a>`,pe=function(e){return e?e.startsWith("http://")||e.startsWith("https://")||e.startsWith("/")?e:`/${e}`:"/logo.png"};let K={html:function(e){let t='<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 justify-items-center">';for(let n of e){const a={...n,image:pe(n.image)};t+=j(me,a)}return t+"</div>"},dom:function(e){return c(K.html(e))}},P={};P.login=async function(e,t){return await V("auth/login",{email:e,password:t})};P.Auth=async function(){return await w("auth")};P.logout=async function(){return await V("auth/logout",{})};let l={},u=[],D=null;async function Y(){if(D===null)try{const e=await P.Auth();e&&e.auth&&e.id?D=e.id:D="guest"}catch{D="guest"}return`cart_${D}`}async function y(){const e=await Y();let t=localStorage.getItem(e);t?u=JSON.parse(t):u=[]}async function A(){const e=await Y();localStorage.setItem(e,JSON.stringify(u))}async function X(){await y(),console.log(" Calcul des totaux avec items:",u);let e=0;for(let a of u)console.log(`  Item: ${a.name} | Prix: ${a.price} | Qté: ${a.quantity} | Ligne: ${a.price*a.quantity}`),e+=a.price*a.quantity;let t=0,n=e+t;return console.log(`  💰 Sous-total: ${e} | Livraison: ${t} | TOTAL: ${n}`),{subtotal:e,shipping:t,total:n}}l.addItem=async function(e){await y();const t=u.find(n=>n.id===e.id);return t?t.quantity<5&&t.quantity++:u.push({id:e.id,name:e.name||"Produit",description:e.description||"",image:e.image||"default.png",price:e.price||0,quantity:1}),await A(),!0};l.updateQuantity=async function(e,t){await y(),console.log(` updateQuantity - ID: ${e}, Nouvelle quantité: ${t}`);let n=u.find(a=>a.id===e);n?(console.log(`  Avant: ${n.name} - Qté: ${n.quantity}`),n.quantity=parseInt(t),console.log(`  Après: ${n.name} - Qté: ${n.quantity}`),await A()):console.error(`  Item avec ID ${e} non trouvé!`)};l.removeItem=async function(e){await y(),u=u.filter(t=>t.id!==e),await A()};l.clear=async function(){u=[],await A()};l.clearCart=async function(){u=[],await A()};l.getItems=async function(){return await y(),u};l.getCart=async function(){return await y(),u};l.getItemCount=async function(){await y();let e=0;for(let t of u)e+=t.quantity;return e};l.getTotalItems=async function(){return await l.getItemCount()};l.getTotalPrice=async function(){return(await X()).total};l.isEmpty=async function(){return await y(),u.length===0};l.removeProduct=async function(e){await y(),u=u.filter(t=>t.id!==e),await A()};l.getState=async function(){let e=await X();return{items:u,itemCount:await l.getItemCount(),subtotal:e.subtotal,shipping:e.shipping,total:e.total,isEmpty:u.length===0}};const fe=`<div class="text-center">
...
</div>`;let E={products:[],user:null};const ge=function(e){return e?e.startsWith("http://")||e.startsWith("https://")?e:`/${e}`:"/placeholder.png"};let F={};F.handler_clickOnProduct=async function(e){let t=e.target.closest("[data-buy]");if(t){e.preventDefault(),e.stopPropagation();let n=t.dataset.buy,a=E.products.find(r=>r.id==n);a&&(await l.addItem({id:a.id,name:a.name,description:a.description||"",image:ge(a.image),price:parseFloat(a.price)||0}),alert(`✅ "${a.name}" ajouté au panier !`))}};F.init=async function(e){try{let t=await P.Auth();t&&t.auth===!0&&(E.user=t)}catch{console.log("Utilisateur non connecté")}return e?.id?E.products=await J.fetchByCategory(e.id):E.products=await B.fetchAll(),S.init(E.products)};let S={};S.init=function(e){let t=S.createPageFragment(e);return S.attachEvents(t),t};S.createPageFragment=function(e){let t=e.length,n=E.user?.username||"",a=j(fe,{username:n,nombre:t}),r=c(a),s=r.querySelector('slot[name="products"]');return s&&s.replaceWith(K.dom(e)),r};S.attachEvents=function(e){return e.firstElementChild.addEventListener("click",F.handler_clickOnProduct),e};function W(e){return F.init(e)}let _={},he=[{id:1,name:"marteau1.png",product_id:1},{id:2,name:"marteau2.png",product_id:1},{id:3,name:"tournevis1.png",product_id:2},{id:4,name:"cle_molette1.png",product_id:3}];_.fetch=async function(e){let t=await w("productimages/"+e);return t==!1?[]:[t]};_.fetchAll=async function(){let e=await w("productimages");return e==!1?he:e};const ye=`<article class="min-h-screen py-24 px-12">
...
`;let Z={html:function(e){return j(ye,e)},dom:function(e){return c(Z.html(e))}};const xe=`<div class="product-gallery grid grid-cols-4 gap-2 mt-4">
  {{gallery}}
</div>`;let ve={dom:function(e){let t=xe,n="";if(e.images&&e.images.length>0)for(let a of e.images)n+=`<img 
          src="${a.src}" 
          alt="Image ${a.id}" 
          class="gallery-image"
          data-image="${a.src}"
        >`;return t=t.replace("{{gallery}}",n),c(t)}};const be=`<div class="p-6">
  <a
    href="#"
    id="btnRetour"
...
`;let d={products:[],productImages:[],currentProduct:null};const ee=function(e){return e?e.startsWith("http://")||e.startsWith("https://")?e:`/${e}`:"/placeholder.png"};d.getProductById=function(e){return d.products.find(t=>t.id==e)};d.getImagesByProductId=function(e){let t=[];for(let n=0;n<d.productImages.length;n++){let a=d.productImages[n];a.product_id==e&&(a.src=ee(a.name),t.push(a))}return t};let x={};x.handler_clickOnProduct=async function(e){e.target.dataset.buy!==void 0&&(e.target.dataset.buy,d.currentProduct&&(await l.addItem({id:d.currentProduct.id,name:d.currentProduct.name,description:d.currentProduct.description||"",image:ee(d.currentProduct.image),price:parseFloat(d.currentProduct.price)||0}),alert(`✅ "${d.currentProduct.name}" ajouté au panier !`)))};x.handler_clickOnBack=function(e){e.preventDefault(),history.back()};x.handler_clickOnGalleryImage=function(e){if(e.target.classList.contains("gallery-image")){const t=document.querySelector("#main-image");t&&(t.src=e.target.dataset.image)}};x.init=async function(e){const t=e.id;d.products=await B.fetchAll(),d.productImages=await _.fetchAll();let n=d.getProductById(t);return n&&(n.images=d.getImagesByProductId(t),d.currentProduct=n),q.init(n)};let q={};q.init=function(e){let t=q.createPageFragment(e);return q.attachEvents(t),t};q.createPageFragment=function(e){let t=c(be),n=Z.dom(e),a=ve.dom(e);const r=t.querySelector('slot[name="detail"]');r&&r.replaceWith(n);const s=t.querySelector('slot[name="gallery"]');return s&&s.replaceWith(a),t};q.attachEvents=function(e){e.querySelector("[data-buy]").addEventListener("click",x.handler_clickOnProduct);const n=e.querySelector("#btnRetour");n&&n.addEventListener("click",x.handler_clickOnBack);const a=e.querySelector(".product-gallery");return a&&a.addEventListener("click",x.handler_clickOnGalleryImage),e};function we(e){return console.log("ProductDetailPage",e),x.init(e)}const Pe=`<div class="max-w-sm mx-auto mt-20 p-6 bg-white rounded-none shadow-none border border-gray-200">
...
</div>`;let te={};te.dom=function(){return c(Pe)};let C={},ke=[{id:1,email:"user1@example.com",username:"user1",password:"password1"},{id:2,email:"user2@example.com",username:"user2",password:"password2"},{id:3,email:"user3@example.com",username:"user3",password:"password3"},{id:4,email:"user4@example.com",username:"user4",password:"password4"}];C.fetch=async function(e){let t=await w("users/"+e);return t==!1?[]:[t]};C.fetchAll=async function(){let e=await w("users");return e==!1?ke:e};C.create=async function(e){return await V("users",e)};C.update=async function(e,t){return await ue("users/"+e,t)};const Ce=`<div class="signin-page">
    <slot name="userForm" id="userForm"></slot>
</div>`;let T={};T.handler_form=async function(e){e.preventDefault();let t=e.target,n=new FormData(t),a=n.get("email"),r=n.get("username"),s=n.get("password");const o=document.querySelector("#successMessage"),i=document.querySelector("#errorMessage");let g={email:a,username:r,password:s},k=await C.create(g);k&&k!==!1?(o&&(o.textContent=`Bienvenue ${r} ! Compte créé avec succès.`,o.classList.remove("hidden")),i&&i.classList.add("hidden"),t.reset(),setTimeout(()=>{window.router?window.router.navigate("/login"):window.location.hash="/login"},2e3)):(i&&(i.textContent="Cet email est déjà utilisé. Veuillez en choisir un autre.",i.classList.remove("hidden")),o&&o.classList.add("hidden"))};T.init=function(){return L.init()};let L={};L.init=function(){let e=L.createPageFragment();return L.attachEvents(e),e};L.createPageFragment=function(){let e=c(Ce),t=te.dom();const n=e.querySelector('slot[name="userForm"]');return n&&n.replaceWith(t),e};L.attachEvents=function(e){return e.querySelector("form").addEventListener("submit",T.handler_form),e};function Ee(){return T.init()}const Se=`<div>
...
</div>`;let ne={html:function(e){return j(Se,e)},dom:function(e){return c(ne.html(e))}};const qe=`<div class="p-12">
  <a
    href="#"
    id="btnRetour"
...
`;let ae={};ae.dom=function(){return c(Ie)};const $e=`<div class="signin-page">
    <slot name="userForm" id="userForm"></slot>
</div>`;let Me={User:null},U={};U.handler_form=async function(e){e.preventDefault();let t=e.target,n=new FormData(t),a=n.get("email"),r=n.get("password");const s=document.querySelector("#successMessage"),o=document.querySelector("#errorMessage");try{console.log("🔐 Tentative de connexion...");let i=await P.login(a,r);console.log("📦 Réponse reçue:",i),i&&i.id?(console.log("✅ Utilisateur authentifié:",i.email),Me.User=i,s&&(s.textContent=`Connexion réussie ! Bienvenue ${i.email}`,s.classList.remove("hidden")),o&&o.classList.add("hidden"),setTimeout(()=>{console.log("🚀 Mise à jour auth et redirection..."),window.router?(window.router.setAuth(!0),window.router.navigate("/profile"),console.log("✅ Navigate appelé")):console.error("❌ window.router n'existe pas !")},500)):(console.log("❌ Échec authentification"),o&&(o.textContent=i?.error||"Email ou mot de passe incorrect.",o.classList.remove("hidden")),s&&s.classList.add("hidden"))}catch(i){console.error("💥 Erreur lors de la connexion :",i),o&&(o.textContent="Une erreur est survenue. Veuillez réessayer.",o.classList.remove("hidden"))}};U.init=function(){return $.init()};let $={};$.init=function(){let e=$.createPageFragment();return $.attachEvents(e),e};$.createPageFragment=function(){let e=c($e),t=ae.dom();const n=e.querySelector('slot[name="userForm"]');return n&&n.replaceWith(t),e};$.attachEvents=function(e){let t=e.querySelector("form");return t&&t.addEventListener("submit",U.handler_form),e};function je(){return U.init()}const Ae=`
...
`;let re={};re.dom=function(){return c(Ae)};const De=`<div class="profile-page">
    <slot name="profile" id="profile"></slot>
</div>`;let O={};O.handler_clickOnLogout=async function(e){e.preventDefault(),console.log("Déconnexion..."),await C.logout(),window.router&&window.router.setAuth(!1),setTimeout(()=>{window.router?window.router.navigate("/login"):window.location.hash="/login"},100)};O.init=function(){return M.init()};let M={};M.init=function(){let e=M.createPageFragment();return M.attachEvents(e),e};M.createPageFragment=function(){let e=c(De),t=re.dom();const n=e.querySelector('slot[name="profile"]');return n&&n.replaceWith(t),e};M.attachEvents=function(e){let t=e.querySelector('button[type="button"]');return t&&t.addEventListener("click",O.handler_clickOnLogout),e};function Re(){return O.init()}const Be=`        
...
</div>`,se=`<div class="text-center py-8">
...
`;let b={cart:[]},f={};f.handleQuantityChange=async function(e){const t=e.target;if(!t.dataset.qty)return;const n=parseInt(t.dataset.qty),a=parseInt(t.value);a>0&&(await l.updateQuantity(n,a),await m.refreshCartContent())};f.handleIncrease=async function(e){const t=e.target.closest("button[data-increase]");if(!t)return;const n=parseInt(t.dataset.increase),a=b.cart.find(r=>r.id===n);a&&(await l.updateQuantity(n,a.quantity+1),await m.refreshCartContent())};f.handleDecrease=async function(e){const t=e.target.closest("button[data-decrease]");if(!t)return;const n=parseInt(t.dataset.decrease),a=b.cart.find(r=>r.id===n);a&&a.quantity>1&&(await l.updateQuantity(n,a.quantity-1),await m.refreshCartContent())};f.handleRemoveItem=async function(e){console.log("handleRemoveItem appelé",e.target);const t=e.target.closest("button[data-remove]");if(console.log("Bouton trouvé:",t),!t)return;const n=parseInt(t.dataset.remove);console.log("ID produit à supprimer:",n),confirm("Voulez-vous retirer cet article du panier ?")&&(await l.removeProduct(n),await m.refreshCartContent())};f.handleClose=function(e){e.preventDefault(),window.router&&window.router.navigate("/products")};f.handlePayment=async function(e){if(e.preventDefault(),b.cart.length===0){alert("Votre panier est vide");return}const t=await l.getTotalPrice();alert(`Paiement de ${t.toFixed(2)}€ en cours...`)};f.init=async function(){return b.cart=await l.getCart(),await m.init()};let m={};m.init=async function(){const e=c(Be),t=e.querySelector('slot[name="cart-items"]');if(!t)return e;if(b.cart.length===0)t.replaceWith(c(se));else{const n=await m.createCartContent();t.replaceWith(n)}return m.attachEvents(e),e};m.createCartContent=async function(){const e=(await l.getTotalPrice()).toFixed(2);let t=Fe;t=t.replaceAll("{{priceTotal}}",e);const n=c(t),a=n.querySelector('slot[name="items"]');if(a){const r=m.createItemsList();a.replaceWith(r)}return n};m.createItemsList=function(){const e=document.createDocumentFragment();return b.cart.forEach(t=>{const n=m.createItemDOM(t);e.appendChild(n)}),e};m.createItemDOM=function(e){let t=Te;t=t.replaceAll("{{id}}",e.id),t=t.replaceAll("{{image}}",e.image||"/placeholder.png"),t=t.replaceAll("{{name}}",e.name),t=t.replaceAll("{{unitPrice}}",e.price.toFixed(2)),t=t.replaceAll("{{quantity}}",e.quantity);const n=(e.price*e.quantity).toFixed(2);return t=t.replaceAll("{{lineTotal}}",n),c(t).firstElementChild};m.attachEvents=function(e){e.querySelectorAll("input[data-qty]").forEach(i=>{i.addEventListener("input",f.handleQuantityChange),i.addEventListener("change",f.handleQuantityChange)}),e.querySelectorAll("button[data-increase]").forEach(i=>{i.addEventListener("click",f.handleIncrease)}),e.querySelectorAll("button[data-decrease]").forEach(i=>{i.addEventListener("click",f.handleDecrease)});const r=e.querySelectorAll("button[data-remove]");console.log("Boutons de suppression trouvés:",r.length),r.forEach(i=>{i.addEventListener("click",f.handleRemoveItem)});const s=e.querySelector("button.text-3xl");s&&s.addEventListener("click",f.handleClose),e.querySelectorAll("button").forEach(i=>{i.textContent.trim()==="PAYER"&&i.addEventListener("click",f.handlePayment)})};m.refreshCartContent=async function(){b.cart=await l.getCart();const e=document.getElementById("cart-container");if(!e){console.error("Conteneur du panier non trouvé");return}const t=document.getElementById("cart-header");if(!t){console.error("Header du panier non trouvé");return}let n=t.nextElementSibling;for(;n;){const a=n;n=n.nextElementSibling,a.remove()}if(b.cart.length===0){const a=c(se);e.appendChild(a)}else{const a=await m.createCartContent();e.appendChild(a),m.attachEvents(e)}};function Ue(){return f.init()}const Oe=`<div style="min-height: 100vh; display: flex; flex-direction: column;">
    ...
    `,Ne=`<nav class="flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-8 xl:gap-12">
  <a
...
`;let R={html:function(e){return j(Ne,e)},dom:function(e){return c(R.html(e))}};const H=`<header class="relative bg-white shadow-sm">
...
`;let Ve={html(){let e=R.html();return console.log(e),H.replace('<slot name="categories-nav"></slot>',e)},dom(){let e=c(H),t=e.querySelector('slot[name="categories-nav"]');t&&t.replaceWith(R.dom());let n=e.querySelector('slot[name="categories-nav-mobile"]');return n&&n.replaceWith(R.dom()),setTimeout(()=>{const a=document.getElementById("burger-menu-btn"),r=document.getElementById("mobile-menu");a&&(a.onclick=()=>{r.classList.toggle("hidden");const s=a.querySelectorAll("span"),o=!r.classList.contains("hidden");s[0].style.transform=o?"rotate(45deg) translate(5px, 7px)":"none",s[1].style.opacity=o?"0":"1",s[2].style.transform=o?"rotate(-45deg) translate(5px, -7px)":"none"})},100),e}};const We=`<footer class="bg-white px-4 md:px-9 py-12 md:py-16 box-border" style="font-family: 'Heebo', sans-serif;">
    ...
    </footer>`;let oe={html:function(e){return j(We,e)},dom:function(e){return c(oe.html(e))}};function _e(){let e=c(Oe),t=Ve.dom(),n=oe.dom();return e.querySelector('slot[name="header"]').replaceWith(t),e.querySelector('slot[name="footer"]').replaceWith(n),e}const ze=` <section>
    <h1>404 - Page non trouvée</h1>
        <p>Cette page n'existe pas</p>
    <nav>
        <a href="/" data-link>Retour à l'accueil</a>
    </nav>
</section>`;function He(){return ze}const p=new ie("app",{loginPath:"/login",basePath:"/Site-commerce/"});window.router=p;const Q=await P.Auth();Q&&Q.auth?p.setAuth(!0):p.setAuth(!1);p.addLayout("/",_e);p.addRoute("/",W);p.addRoute("/about",ce);p.addRoute("/products",W);p.addRoute("/login",je,{useLayout:!1});p.addRoute("/data",Le,{useLayout:!1});p.addRoute("/cart",Ue,{requireAuth:!0});p.addRoute("/signup",Ee,{useLayout:!1});p.addRoute("/products/categories/:id",W);p.addRoute("/profile",Re,{requireAuth:!0});p.addRoute("/products/:id/:slug",we);p.addRoute("*",He);p.start();
