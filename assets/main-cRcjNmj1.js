...
</a>`,he=function(e){return e?e.startsWith("http://")||e.startsWith("https://")||e.startsWith("/")?e:`/${e}`:"/logo.png"};let Y={html:function(e){let t='<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 justify-items-center">';for(let n of e){const a={...n,image:he(n.image)};t+=A(ge,a)}return t+"</div>"},dom:function(e){return c(Y.html(e))}},P={};P.login=async function(e,t){return await W("auth/login",{email:e,password:t})};P.Auth=async function(){return await w("auth")};P.logout=async function(){return await W("auth/logout",{})};let l={},u=[],R=null;async function X(){if(R===null)try{const e=await P.Auth();e&&e.auth&&e.id?R=e.id:R="guest"}catch{R="guest"}return`cart_${R}`}async function y(){const e=await X();let t=localStorage.getItem(e);t?u=JSON.parse(t):u=[]}async function j(){const e=await X();localStorage.setItem(e,JSON.stringify(u))}async function Z(){await y(),console.log(" Calcul des totaux avec items:",u);let e=0;for(let a of u)console.log(`  Item: ${a.name} | Prix: ${a.price} | Qté: ${a.quantity} | Ligne: ${a.price*a.quantity}`),e+=a.price*a.quantity;let t=0,n=e+t;return console.log(`  💰 Sous-total: ${e} | Livraison: ${t} | TOTAL: ${n}`),{subtotal:e,shipping:t,total:n}}l.addItem=async function(e){await y();const t=u.find(n=>n.id===e.id);return t?t.quantity<5&&t.quantity++:u.push({id:e.id,name:e.name||"Produit",description:e.description||"",image:e.image||"default.png",price:e.price||0,quantity:1}),await j(),!0};l.updateQuantity=async function(e,t){await y(),console.log(` updateQuantity - ID: ${e}, Nouvelle quantité: ${t}`);let n=u.find(a=>a.id===e);n?(console.log(`  Avant: ${n.name} - Qté: ${n.quantity}`),n.quantity=parseInt(t),console.log(`  Après: ${n.name} - Qté: ${n.quantity}`),await j()):console.error(`  Item avec ID ${e} non trouvé!`)};l.removeItem=async function(e){await y(),u=u.filter(t=>t.id!==e),await j()};l.clear=async function(){u=[],await j()};l.clearCart=async function(){u=[],await j()};l.getItems=async function(){return await y(),u};l.getCart=async function(){return await y(),u};l.getItemCount=async function(){await y();let e=0;for(let t of u)e+=t.quantity;return e};l.getTotalItems=async function(){return await l.getItemCount()};l.getTotalPrice=async function(){return(await Z()).total};l.isEmpty=async function(){return await y(),u.length===0};l.removeProduct=async function(e){await y(),u=u.filter(t=>t.id!==e),await j()};l.getState=async function(){let e=await Z();return{items:u,itemCount:await l.getItemCount(),subtotal:e.subtotal,shipping:e.shipping,total:e.total,isEmpty:u.length===0}};const ye=`<div class="text-center">
...
</div>`;let S={products:[],user:null};const xe=function(e){return e?e.startsWith("http://")||e.startsWith("https://")?e:`/${e}`:"/placeholder.png"};let T={};T.handler_clickOnProduct=async function(e){let t=e.target.closest("[data-buy]");if(t){e.preventDefault(),e.stopPropagation();let n=t.dataset.buy,a=S.products.find(r=>r.id==n);a&&(await l.addItem({id:a.id,name:a.name,description:a.description||"",image:xe(a.image),price:parseFloat(a.price)||0}),alert(`✅ "${a.name}" ajouté au panier !`))}};T.init=async function(e){try{let t=await P.Auth();t&&t.auth===!0&&(S.user=t)}catch{console.log("Utilisateur non connecté")}return e?.id?S.products=await K.fetchByCategory(e.id):S.products=await D.fetchAll(),E.init(S.products)};let E={};E.init=function(e){let t=E.createPageFragment(e);return E.attachEvents(t),t};E.createPageFragment=function(e){let t=e.length,n=S.user?.username||"",a=A(ye,{username:n,nombre:t}),r=c(a),s=r.querySelector('slot[name="products"]');return s&&s.replaceWith(Y.dom(e)),r};E.attachEvents=function(e){return e.firstElementChild.addEventListener("click",T.handler_clickOnProduct),e};function _(e){return T.init(e)}let z={},ve=[{id:1,name:"marteau1.png",product_id:1},{id:2,name:"marteau2.png",product_id:1},{id:3,name:"tournevis1.png",product_id:2},{id:4,name:"cle_molette1.png",product_id:3}];z.fetch=async function(e){let t=await w("productimages/"+e);return t==!1?[]:[t]};z.fetchAll=async function(){let e=await w("productimages");return e==!1?ve:e};const be=`<article class="min-h-screen py-24 px-12">
...
`;let ee={html:function(e){return A(be,e)},dom:function(e){return c(ee.html(e))}};const we=`<div class="product-gallery grid grid-cols-4 gap-2 mt-4">
  {{gallery}}
...
`;let d={products:[],productImages:[],currentProduct:null};const te=function(e){return e?e.startsWith("http://")||e.startsWith("https://")?e:`/${e}`:"/placeholder.png"};d.getProductById=function(e){return d.products.find(t=>t.id==e)};d.getImagesByProductId=function(e){let t=[];for(let n=0;n<d.productImages.length;n++){let a=d.productImages[n];a.product_id==e&&(a.src=te(a.name),t.push(a))}return t};let x={};x.handler_clickOnProduct=async function(e){e.target.dataset.buy!==void 0&&(e.target.dataset.buy,d.currentProduct&&(await l.addItem({id:d.currentProduct.id,name:d.currentProduct.name,description:d.currentProduct.description||"",image:te(d.currentProduct.image),price:parseFloat(d.currentProduct.price)||0}),alert(`✅ "${d.currentProduct.name}" ajouté au panier !`)))};x.handler_clickOnBack=function(e){e.preventDefault(),history.back()};x.handler_clickOnGalleryImage=function(e){if(e.target.classList.contains("gallery-image")){const t=document.querySelector("#main-image");t&&(t.src=e.target.dataset.image)}};x.init=async function(e){const t=e.id;d.products=await D.fetchAll(),d.productImages=await z.fetchAll();let n=d.getProductById(t);return n&&(n.images=d.getImagesByProductId(t),d.currentProduct=n),q.init(n)};let q={};q.init=function(e){let t=q.createPageFragment(e);return q.attachEvents(t),t};q.createPageFragment=function(e){let t=c(ke),n=ee.dom(e),a=Pe.dom(e);const r=t.querySelector('slot[name="detail"]');r&&r.replaceWith(n);const s=t.querySelector('slot[name="gallery"]');return s&&s.replaceWith(a),t};q.attachEvents=function(e){e.querySelector("[data-buy]").addEventListener("click",x.handler_clickOnProduct);const n=e.querySelector("#btnRetour");n&&n.addEventListener("click",x.handler_clickOnBack);const a=e.querySelector(".product-gallery");return a&&a.addEventListener("click",x.handler_clickOnGalleryImage),e};function Ce(e){return console.log("ProductDetailPage",e),x.init(e)}const Se=`<div class="max-w-sm mx-auto mt-20 p-6 bg-white rounded-none shadow-none border border-gray-200">
...
</div>`;let ae={html:function(e){return A(Ie,e)},dom:function(e){return c(ae.html(e))}};const $e=`<div class="p-12">
  <a
    href="#"
    id="btnRetour"
...
`;let re={};re.dom=function(){return c(Ae)};const je=`<div class="signin-page">
    <slot name="userForm" id="userForm"></slot>
</div>`;let Re={User:null},U={};U.handler_form=async function(e){e.preventDefault();let t=e.target,n=new FormData(t),a=n.get("email"),r=n.get("password");const s=document.querySelector("#successMessage"),o=document.querySelector("#errorMessage");try{console.log("🔐 Tentative de connexion...");let i=await P.login(a,r);console.log("📦 Réponse reçue:",i),i&&i.id?(console.log("✅ Utilisateur authentifié:",i.email),Re.User=i,s&&(s.textContent=`Connexion réussie ! Bienvenue ${i.email}`,s.classList.remove("hidden")),o&&o.classList.add("hidden"),setTimeout(()=>{console.log("🚀 Mise à jour auth et redirection..."),window.router?(window.router.setAuth(!0),window.router.navigate("/profile"),console.log("✅ Navigate appelé")):console.error("❌ window.router n'existe pas !")},500)):(console.log("❌ Échec authentification"),o&&(o.textContent=i?.error||"Email ou mot de passe incorrect.",o.classList.remove("hidden")),s&&s.classList.add("hidden"))}catch(i){console.error("💥 Erreur lors de la connexion :",i),o&&(o.textContent="Une erreur est survenue. Veuillez réessayer.",o.classList.remove("hidden"))}};U.init=function(){return $.init()};let $={};$.init=function(){let e=$.createPageFragment();return $.attachEvents(e),e};$.createPageFragment=function(){let e=c(je),t=re.dom();const n=e.querySelector('slot[name="userForm"]');return n&&n.replaceWith(t),e};$.attachEvents=function(e){let t=e.querySelector("form");return t&&t.addEventListener("submit",U.handler_form),e};function Be(){return U.init()}const De=`
...
`;let se={};se.dom=function(){return c(De)};const Te=`<div class="profile-page">
    <slot name="profile" id="profile"></slot>
</div>`;let O={};O.handler_clickOnLogout=async function(e){e.preventDefault(),console.log("Déconnexion..."),await C.logout(),window.router&&window.router.setAuth(!1),setTimeout(()=>{window.router?window.router.navigate("/login"):window.location.hash="/login"},100)};O.init=function(){return M.init()};let M={};M.init=function(){let e=M.createPageFragment();return M.attachEvents(e),e};M.createPageFragment=function(){let e=c(Te),t=se.dom();const n=e.querySelector('slot[name="profile"]');return n&&n.replaceWith(t),e};M.attachEvents=function(e){let t=e.querySelector('button[type="button"]');return t&&t.addEventListener("click",O.handler_clickOnLogout),e};function Fe(){return O.init()}const Ue=`        
...
</div>`,oe=`<div class="text-center py-8">
...
`;let b={cart:[]},f={};f.handleQuantityChange=async function(e){const t=e.target;if(!t.dataset.qty)return;const n=parseInt(t.dataset.qty),a=parseInt(t.value);a>0&&(await l.updateQuantity(n,a),await m.refreshCartContent())};f.handleIncrease=async function(e){const t=e.target.closest("button[data-increase]");if(!t)return;const n=parseInt(t.dataset.increase),a=b.cart.find(r=>r.id===n);a&&(await l.updateQuantity(n,a.quantity+1),await m.refreshCartContent())};f.handleDecrease=async function(e){const t=e.target.closest("button[data-decrease]");if(!t)return;const n=parseInt(t.dataset.decrease),a=b.cart.find(r=>r.id===n);a&&a.quantity>1&&(await l.updateQuantity(n,a.quantity-1),await m.refreshCartContent())};f.handleRemoveItem=async function(e){console.log("handleRemoveItem appelé",e.target);const t=e.target.closest("button[data-remove]");if(console.log("Bouton trouvé:",t),!t)return;const n=parseInt(t.dataset.remove);console.log("ID produit à supprimer:",n),confirm("Voulez-vous retirer cet article du panier ?")&&(await l.removeProduct(n),await m.refreshCartContent())};f.handleClose=function(e){e.preventDefault(),window.router&&window.router.navigate("/products")};f.handlePayment=async function(e){if(e.preventDefault(),b.cart.length===0){alert("Votre panier est vide");return}const t=await l.getTotalPrice();alert(`Paiement de ${t.toFixed(2)}€ en cours...`)};f.init=async function(){return b.cart=await l.getCart(),await m.init()};let m={};m.init=async function(){const e=c(Ue),t=e.querySelector('slot[name="cart-items"]');if(!t)return e;if(b.cart.length===0)t.replaceWith(c(oe));else{const n=await m.createCartContent();t.replaceWith(n)}return m.attachEvents(e),e};m.createCartContent=async function(){const e=(await l.getTotalPrice()).toFixed(2);let t=Oe;t=t.replaceAll("{{priceTotal}}",e);const n=c(t),a=n.querySelector('slot[name="items"]');if(a){const r=m.createItemsList();a.replaceWith(r)}return n};m.createItemsList=function(){const e=document.createDocumentFragment();return b.cart.forEach(t=>{const n=m.createItemDOM(t);e.appendChild(n)}),e};m.createItemDOM=function(e){let t=Ne;t=t.replaceAll("{{id}}",e.id),t=t.replaceAll("{{image}}",e.image||"/placeholder.png"),t=t.replaceAll("{{name}}",e.name),t=t.replaceAll("{{unitPrice}}",e.price.toFixed(2)),t=t.replaceAll("{{quantity}}",e.quantity);const n=(e.price*e.quantity).toFixed(2);return t=t.replaceAll("{{lineTotal}}",n),c(t).firstElementChild};m.attachEvents=function(e){e.querySelectorAll("input[data-qty]").forEach(i=>{i.addEventListener("input",f.handleQuantityChange),i.addEventListener("change",f.handleQuantityChange)}),e.querySelectorAll("button[data-increase]").forEach(i=>{i.addEventListener("click",f.handleIncrease)}),e.querySelectorAll("button[data-decrease]").forEach(i=>{i.addEventListener("click",f.handleDecrease)});const r=e.querySelectorAll("button[data-remove]");console.log("Boutons de suppression trouvés:",r.length),r.forEach(i=>{i.addEventListener("click",f.handleRemoveItem)});const s=e.querySelector("button.text-3xl");s&&s.addEventListener("click",f.handleClose),e.querySelectorAll("button").forEach(i=>{i.textContent.trim()==="PAYER"&&i.addEventListener("click",f.handlePayment)})};m.refreshCartContent=async function(){b.cart=await l.getCart();const e=document.getElementById("cart-container");if(!e){console.error("Conteneur du panier non trouvé");return}const t=document.getElementById("cart-header");if(!t){console.error("Header du panier non trouvé");return}let n=t.nextElementSibling;for(;n;){const a=n;n=n.nextElementSibling,a.remove()}if(b.cart.length===0){const a=c(oe);e.appendChild(a)}else{const a=await m.createCartContent();e.appendChild(a),m.attachEvents(e)}};function Ve(){return f.init()}const We=`<div style="min-height: 100vh; display: flex; flex-direction: column;">
    ...
    `,_e=`<nav class="flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-8 xl:gap-12">
  <a
...
`;let B={html:function(e){return A(_e,e)},dom:function(e){return c(B.html(e))}};const Q=`<header class="relative bg-white shadow-sm">
...
`;let ze={html(){let e=B.html();return console.log(e),Q.replace('<slot name="categories-nav"></slot>',e)},dom(){let e=c(Q),t=e.querySelector('slot[name="categories-nav"]');t&&t.replaceWith(B.dom());let n=e.querySelector('slot[name="categories-nav-mobile"]');return n&&n.replaceWith(B.dom()),setTimeout(()=>{const a=document.getElementById("burger-menu-btn"),r=document.getElementById("mobile-menu");a&&(a.onclick=()=>{r.classList.toggle("hidden");const s=a.querySelectorAll("span"),o=!r.classList.contains("hidden");s[0].style.transform=o?"rotate(45deg) translate(5px, 7px)":"none",s[1].style.opacity=o?"0":"1",s[2].style.transform=o?"rotate(-45deg) translate(5px, -7px)":"none"})},100),e}};const He=`<footer class="bg-white px-4 md:px-9 py-12 md:py-16 box-border" style="font-family: 'Heebo', sans-serif;">
    ...
    </footer>`;let ie={html:function(e){return A(He,e)},dom:function(e){return c(ie.html(e))}};function Qe(){let e=c(We),t=ze.dom(),n=ie.dom();return e.querySelector('slot[name="header"]').replaceWith(t),e.querySelector('slot[name="footer"]').replaceWith(n),e}const Ge=` <section>
    <h1>404 - Page non trouvée</h1>
        <p>Cette page n'existe pas</p>
    <nav>
        <a href="/" data-link>Retour à l'accueil</a>
    </nav>
</section>`;function Je(){return Ge}const p=new le("app",{loginPath:"/login",basePath:"/Site-commerce/"});window.router=p;const G=await P.Auth();G&&G.auth?p.setAuth(!0):p.setAuth(!1);p.addLayout("/",Qe);p.addRoute("/",_);p.addRoute("/about",ue);p.addRoute("/products",_);p.addRoute("/login",Be,{useLayout:!1});p.addRoute("/data",Me,{useLayout:!1});p.addRoute("/cart",Ve,{requireAuth:!0});p.addRoute("/signup",Le,{useLayout:!1});p.addRoute("/products/categories/:id",_);p.addRoute("/profile",Fe,{requireAuth:!0});p.addRoute("/products/:id/:slug",Ce);p.addRoute("*",Je);p.start();
